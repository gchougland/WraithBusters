...
/**
 * Hides mana orb entities from human players. Ghosts and creative-mode players still see them.
 */
public final class ManaOrbVisibilitySystem extends EntityTickingSystem<EntityStore> {
    @Nonnull
    ...
    public void tick(
        ...
        Set<Ref<EntityStore>> orbRefs = ManaPickupService.activeOrbRefs(session.getSessionId());
        if (orbRefs.isEmpty()) {
            return;
        }
        ...
        if (ManaPickupService.canPlayerSeeManaOrbs(session, playerRef.getUuid(), player)) {
            return;
        }
        ...
        while (iterator.hasNext()) {
            ...
            if (orbRefs.contains(visibleRef)) {
                viewer.hiddenCount++;
                iterator.remove();
            }
        }
